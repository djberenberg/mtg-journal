...
try {
  ...
  ws.addEventListener('message', (m) => { const d = JSON.parse(m.data); if (d.method === 'Page.javascriptDialogOpening') send('Page.handleJavaScriptDialog', { accept: true }); });
  ...
  let u = await ui();
  check('a fresh page: turn 1, my turn, 20/20, one log line, undo disabled, search focused', u.turn === 'turn 1 · my turn' && u.me === '20' && u.opp === '20' && u.log.length === 1 && u.undo && await evalJs(`document.activeElement.id`) === 'q', JSON.stringify(u));
  ...
  check('undo brings the removed card back', (await zone('me', 'graveyard')).length === 1);
  await click(`.card[data-uid="${hand[0].uid}"] button[data-act="remove"]`); await sleep(50);
  check('(removed again; state matches)', JSON.stringify(await state()) === before);
  ...
  // new game (confirm dialog accepted by the handler above)
  await click('#new'); await sleep(200); u = await ui();
  check('new game clears the table and resets everything', (await state()).cards.length === 0 && u.turn === 'turn 1 · my turn' && u.opp === '20' && u.log.length === 1, JSON.stringify({ turn: u.turn, opp: u.opp, log: u.log }));
  check('the table has no card elements left', (await evalJs(`document.querySelectorAll('.card').length`)) === 0);
